<script>
/*

Copyright (c) 2020, all rights reserved.

All the information provided by this platform is protected by international laws related  to 
industrial property, intellectual property, copyright and relative international laws. 
All intellectual or industrial property rights of the code, texts, trade mark, design, 
pictures and any other information belongs to the owner of this platform.

Without the written permission of the owner, any replication, modification,
transmission, publication is strictly forbidden.

For more information read the license file including with this software.

// · ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~
// · 

*/


// · List of Imported Components
// · ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~
import componentForm from '../components/form.vue'


// · 
// · ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~     ~·~


export default {
    props: {

    },

    components: {
        'component-form': componentForm
    },

    // @return [Object] Data used by this component's methods
    // @description Returns the data needed for this component to work properly
    // @data_variable main_route [String] the main route to which this component connects to the lesli API
    // @data_variable dashboards [Object] An object representing a Dashboards, with
    //      the same params as the associated rails model
    // @data_variable dashboards_id [String|Integer] The id of the Dashboards, as
    //      obtained from the route using the *Vue-router* *params* 
    data(){
        return {
            main_route: '/driver/dashboards',
            dashboards: null,
            dashboards_id: null
        }
    },

    // @return [void]
    // @description Executes the necessary methods needed to initialize this component
    mounted(){
        this.setdashboardsId()
        this.getdashboards()
    },

    methods: {

        // @return [void]
        // @description Retrieves the id of the Dashboards and stores it in the data variable dashboards_id
        // @example
        //      console.log(this.dashboards_id) // will display null
        //      this.setdashboardsId()
        //      console.log(this.dashboards_id) // will display a number, like 5
        setdashboardsId(){
            this.dashboards_id = this.$route.params.id
        },

        // @return [void]
        // @description Connects to the backend using HTTP and retrieves the Dashboards associated to
        //      the variable *Dashboards_id*. If the HTTP request fails, an error message is shown
        // @example
        //      console.log(this.dashboards) // will display null
        //      this.getdashboards()
        //      console.log(this.dashboards) // will display an object representation of the Dashboards
        getdashboards(){
            this.http.get(`${this.main_route}/${this.dashboards_id}.json`).then(result => {
                if (result.successful) {
                    this.dashboards = result.data
                }else{
                    this.alert(result.error.message, 'danger')
                }
            }).catch(error => {
                console.log(error)
            })
        }
    }
}
</script>
<template>
    <section class="section">
        <component-form v-if="dashboards" :dashboards="dashboards" view-type="edit"/>
        <component-layout-data-loading v-else size="is-medium" />
    </section>
</template>
