...
RSpec.describe CloudDriver::Event::AttendantServices, type: :model do
    ...
    it "is expected to not create an event attendant for nil event" do
        current_user = FactoryBot.create(:user)

        event_id = CloudDriver::Event.all.count + 1

        event_attendant_params = {
            users_id: User.all.count
        }
    ...
    it "is expected to not create an event attendant for nil user" do
        current_user = FactoryBot.create(:user)
